...
|---|---|---|---|---|
| `header` | Site identity and navigation | Any multi-section or multi-page site | Logo/name, verified page targets | Seen in API contract |
...
| `description` | Long-form game or offer explanation | Context improves purchase confidence | Approved HTML copy | Seen in default template docs |
| `packs` | Present purchasable packs | A template already uses packs and its data shape is verified | Pack/catalog references, copy | Seen in default template docs |
...
| `gallery` | Screenshots or promotional media | PC portal, event, or visually rich game | Images and alt text | Seen in default template docs |
| `requirements` | Platform/system requirements | PC or console information page | Verified requirements by platform | Seen in default template docs |
| `faq` | Purchase and account questions | Supportable answers exist | Approved question/answer pairs | Seen in CLI docs |
| `footer` | Legal/support/secondary links | Any complete site | Link labels and verified URLs | Seen in default template docs |
